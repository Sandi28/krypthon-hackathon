...
def create():
    ...
    Label(cr,text='Note: Enter the candidate names one by one with commas').grid(row=4,column=2)
    Label(cr,text='eg: ram,sadi,..').grid(row=5,column=2)
...
def about():
    messagebox.showinfo('About','Developed by Sandiya, Varsheni, Swarnaa malika, Srimathi')
...
home['bg'] = '#49A'
Label(home,text='Voting program made in python',font='MSSansSerif 10 bold',bg='#49A').grid(row=1,column=2)
Button(home,text='Create a new Poll',command=create).grid(row=3,column=2)
